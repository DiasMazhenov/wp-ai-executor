...
Глобально относительно цели проекта мы пока топчемся на месте: один
подтверждённый process retry не превращает локальный успех в доказанный
прогресс всей native Elementor-системы. Не закрыты свежие v114 FAQ/portfolio,
EJ-128, пользовательский контрольный блок, полный retry с пользовательскими
правками и public acceptance нового draft из-за пустого preview. Нужны реальные
предложения по реальному глобальному прогрессу — изменения, которые расширяют
проверенное покрытие и устраняют общие причины, а не очередные точечные правки.

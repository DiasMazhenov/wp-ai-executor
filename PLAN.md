...
- В `v02.08.78` Elementor-чат показывает детали отказа action-пайплайна и позволяет скопировать текущий диалог как лог; промпты по-прежнему не сохраняются на сервере.

...
- Адаптировать content-only Team briefs формата «имя, роль. описание» в
  отдельные карточки CopyElement Team, не превращая их в плоский текст.
- Адаптировать Image Box briefs к исходной nested-carousel композиции:
  сохранить изображения и Flex-карточки, добавить заголовок и удалить лишние
  пустые слайды.
- Предпочитать CopyElement для обычных запросов, оставляя Vocario только при
  явном запросе; выровнять сохранённый бейдж по выравниванию исходного контента.
- Пройти lint/контрактные тесты, доставить через WP Pusher и повторить Team,
  Image Box, Testimonials и About через Browser Use со screenshot, scoped JSON,

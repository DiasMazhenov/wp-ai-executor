...
Release state: local `v02.09.34` prepared, including the LLM proxy, Elementor editor chat,
...
The latest local release `v02.09.34` is published on GitHub `main`, but the
currently open Elementor editor was verified on 2026-08-30 and still reports
live plugin `v02.09.28` and guide `v02.05.72`; content-fidelity and provider
retry fixes therefore remain pending live deployment. Browser tests on the old
live release reproduced template-content substitution and empty provider
responses. Public verification confirmed that the removed

...
assert.match(bootstrap, /Version:\s+v02\.10\.83/);
assert.match(bootstrap, /const WPAE_VERSION = 'v02\.10\.83'/);
...
assert.match(llm, /function wpae_llm_normalize_hero_composition\( array \$elements, int &\$changed = 0, string \$message = '', bool \$clean_trusted_source = false \)/);
assert.match(llm, /wpae-hero-content-shell/);
assert.match(llm, /wpae_llm_normalize_hero_composition\( \$action\['elements'\], \$hero_composition_changed, \$message, ! empty\( \$selected_library\['trusted_bundled'\] \) \)/);

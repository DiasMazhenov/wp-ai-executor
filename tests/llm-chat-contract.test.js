...
assert.match(llm, /card_widget.*\$action_archetype === 'testimonials' \? 'heading \+ text-editor' : 'icon-box'/);
assert.match(llm, /decorative-only/);
assert.match(guide, /Testimonial cards are an explicit exception/);
assert.match(guide, /do not use icon-box/);
assert.match(guide, /testimonial cards are the semantic exception/);
...
assert.match(llm, /\$settings\['title_typography_font_size'\] = \[ 'unit' => 'rem', 'size' => 1 \]/);

...
function wpae_agent_guide(): array {
    return [
        ...
        'chat_quality_contract' => [
            ...
            'visual_grammar' => 'Every generated block receives one compact outlined rounded native badge container with a native heading label inside. Icon-box is forbidden: card headings use native heading and any card icon uses a separate native icon widget. Testimonial cards use a native heading for author/company and a native text-editor for the quote; an icon is decorative only. The bento grid container stays transparent; card backgrounds belong to the cards. This is a plugin default and must not be required in the user prompt.',
        ...
        'workflow' => [
            ...
            '4e. For natural-language block requests, infer the block archetype from the user intent (hero, benefits, pricing, testimonials, FAQ, process, CTA, portfolio) and choose the corresponding native Elementor widgets. Do not force every block into the same heading/text/button composition; use the matching recipe or native widget when available, while preserving the common design system and validation contract.',
            '4f. If the provider returns an unusable action and the bounded repair pass also fails, accept a deterministic fallback only when the response marks its fallback_archetype and the fallback still passes all Elementor write, design-system, native-content, and visual checks. Report that fallback explicitly; never claim that the provider generated the block.',
            '4g. Repeating content such as testimonials, benefits, pricing packages, cases, and process steps should default to separate native Flexbox card containers with their own editable background, border, radius, padding, responsive width, and gap settings. Do not render repeated items as one flat text stream unless the user explicitly requests a plain list.',
            '4h. Match typography to semantic role in repeated blocks: section headings may use h2/h3, but testimonial author names and compact card labels must use native heading h5/h6 or body text. Never use display-scale h1/h2 typography for author names, metadata, or short labels; keep quote copy in text-editor/testimonial widgets.',
            '4i. The plugin applies a native bento layout by default to repeatable block archetypes: separate editable card containers, responsive widths, wrapping, and no more than four items per row. Do not flatten repeated content into one stream; explicit user instructions for a different composition take precedence.',
            '4j. Bento grids must contain repeatable items only. Keep the section heading, intro copy, and primary CTA outside the card grid; never turn a heading, description, icon-list, or CTA into a standalone card merely to reach a column count.',
            '4k. The normalizer is the compatibility boundary: before saving, migrate legacy container alignment/gap aliases to current Flexbox controls (flex_justify_content, flex_align_items, flex_gap and responsive variants), set container_type=flex, and ensure native button background/text colors are explicit design-token settings so theme defaults cannot override the generated design.',
            '4l. For a new natural-language block request, retrieve only compatible approved/published records from the private block library, rank them by archetype and prompt terms, adapt repeated-card content through native widgets, and require native shape plus content-fidelity checks before insertion. Skip retrieval for selected edits and Vision repair passes; expose only safe candidate metadata in the chat trace, never raw library JSON.',
            '4m. For horizontal process/timeline cards only, use the supplied reference JSON card as the structural and visual contract: the section begins with the standard native ПРОЦЕСС badge and a native section heading, then a responsive items row; each step is its own native Flexbox card with class wpae-process-content; inside it place a marker row with the blue native marker and, except on the last step, a native Elementor Divider, followed by a native heading and text-editor. Use a white card, 1px #dbe3f0 border, 20px radius, 1rem .75rem 1.25rem padding, 22% desktop width and 100% mobile width. Do not create a separate shared rail/cards row and never use an HTML widget or CSS-only connector. Apply this rule only to the horizontal timeline; do not change the existing vertical timeline card settings.',
            ...
            '4e. LLM chat is a server-side proxy for configured OpenAI-compatible providers. Ordinary chat is advisory; explicit action requests may insert only new elements when elementor_writes is enabled and always pass structured update, preflight, protected-zone, visual-regression, and rollback checks. Delete and replace actions are forbidden. Tutorial text is rejected for action requests.',
        ...
        'frontend_design' => [
            ...
            'design_quality_bar' => [
                ...
                'Generated block grammar is mandatory: one compact outlined rounded badge container per block, a native icon beside every repeatable card heading, and no background color on the container that holds the cards.',
            ],
            ...
            'ai_vision_rule' => 'When AI Vision is enabled, use it as a second visual review layer for desktop/mobile screenshots. The floating Elementor chat must analyze the refreshed preview after a successful write; full-block generations reject scores below 75 or major/critical findings, roll back the write snapshot, and allow at most two repair passes, while selected-tree patches remain advisory. If Vision is temporarily unavailable, retain the saved generation and request manual review; only a received quality failure triggers rollback. Do not treat a Vision pass as proof of DOM validity, Elementor editability, computed CSS, keyboard behavior, or animation correctness.',
        ],
        ...
        'wordpress_elementor' => [
            ...
            'filesystem_policy' => [
                ...
                'package_update_policy' => [
                    ...
                    'validation' => 'ZIP must include wpae-package.json with SHA-256 for every file; only wp-ai-executor.php, includes/*.php|json, and assets/*.js|css paths are accepted.',
        ...
        'runtime_elementor_validation' => [
            ...
            'native_template_policy' => [
                'endpoints' => [
                    'list' => 'GET /wp-json/ai-executor/v1/elementor/templates',
                    'get' => 'GET /wp-json/ai-executor/v1/elementor/templates/{id}',
                ],
                'authentication' => 'X-AI-Key is required. These endpoints are read-only and never expose the key to browser JavaScript.',
                'workflow' => [
                    'Search templates before generating a common section from scratch.',
                    'Inspect elementor_data, normalized_elementor_data, page_settings, and compatibility.',
                    'Use normalized_elementor_data with /elementor/validate before a structured write.',
                ],
                'filters' => [
                    'q' => 'Title search.',
                    'type' => 'Elementor template type, e.g. page, section, header, footer.',
            ...
            'block_library_policy' => [
                ...
                'instantiate_modes' => [
                    ...
                    'compatibility' => 'Normalize legacy sections/columns and widget keys, add safe Flexbox baseline settings, rekey IDs, and validate.',
                    'adapt' => 'Run compatibility mode and deterministically map semantic colors, typography, spacing, and radii through native Elementor settings. Inspect token_map.mapped, unmatched, collisions, evidence, native_paths, and protected_skips before insertion.',
                ],
                'agent_workflow' => [
                    'Search /elementor/blocks before generating a common section from scratch.',
                    'Inspect the full stored block and compatibility report.',
                    'Choose preserve, compatibility, or adapt according to the user request; never silently restyle a foreign block.',
                    'Merge the returned elementor_data into the target page.',
                    'Run /elementor/validate and the target write endpoint with dry_run=true before saving.',
                    'Preserve protected HTML/WebGL/Three.js/GSAP/canvas enhancement zones.',
                ],
            ],
            'rollback_policy' => [
            ...
            'elementor_data_rules' => [
                ...
                'Generated blocks use one compact outlined rounded badge. Icon-box is forbidden: card headings use native heading, and any card icon uses a separate native icon widget. Testimonial authors use native heading plus native text-editor quote.',
                ...
                'Use current Flexbox Container settings for layout: container_type=flex, content_width, width or boxed_width, min_height, flex_direction, flex_justify_content, flex_align_items, flex_gap, flex_wrap, padding, margin, and responsive variants. Legacy aliases justify_content, align_items, and gap are not reliable for modern containers.',
...
Current runtime notes: rollback restores only plugin-managed Elementor/WPAE meta plus the post record and does not remove unrelated third-party post meta; /elementor/page attempts to delete a newly created page if Elementor metadata saving fails and reports cleanup.created_post_deleted; /media/upload rejects files whose binary signature does not match mime_type; /exports/create stores short-lived JSON in wp_options and returns an authenticated /exports/{id} endpoint, never a public uploads file; use GET /exports for metadata and POST /exports/prune to remove expired export records.
   AI Vision note: when the ai_vision capability is enabled, render desktop and mobile screenshots in the agent environment, call /vision/analyze or submit a structured result to /vision/report, then call /vision/page-review. The floating Elementor chat reviews its refreshed preview through /llm/vision-review after each successful action. New full-block generations treat received quality findings as a bounded quality gate: quality failures trigger rollback and at most two repair passes, while a temporarily unavailable review is retained for manual verification; selected-tree patches remain advisory. Vision returns vision_score, findings, confidence, and must_fix; it is additional evidence and never replaces deterministic audits, native Elementor checks, or public browser verification. Use transaction_vision_review=true with vision_report_id when atomic rollback on critical findings is explicitly required.
   LLM chat note: /llm/chat is a server-side proxy for the site owner's configured OpenAI-compatible provider. Ordinary chat is advisory. For explicit action requests such as "create a hero", the model must return only the bounded insert_elements JSON command; the plugin inserts new elements only when elementor_writes is enabled and routes them through the existing structured Elementor update, preflight, protected-zone, visual-regression, and rollback workflow. Tutorial text, delete commands, and replace commands are rejected. Provider keys are never exposed.
   Generated component grammar note: every generated block gets one compact outlined rounded native badge container with a heading label inside. Icon-box is forbidden: card headings use a native heading and any card icon uses a separate native icon widget. Testimonial cards use a native heading for author/company, the quote is a native text-editor, and any quote/icon marker is decorative only. The card-grid container remains transparent. The plugin enforces this after provider decoding, so users do not need to write technical layout instructions.
...
Native Elementor template note: use GET /elementor/templates and GET /elementor/templates/{id} with X-AI-Key to inspect the site's saved elementor_library templates before generating a common section from scratch. These endpoints are read-only; inspect the returned elementor_data, normalized_elementor_data, page_settings, and compatibility report, then validate the selected normalized data before a structured write. Never send X-AI-Key to browser JavaScript.
Elementor block library note: before generating a common section from scratch, search GET /elementor/blocks. The library accepts raw native Elementor JSON and wpae-elementor-block-v1 wrappers, including foreign blocks with no design_system_id. Use preserve to keep the original design, compatibility to normalize for current Flexbox structured writes, or adapt to map the active design system deterministically through native settings. Never silently restyle a foreign block. In adapt mode inspect token_map.mapped, unmatched, collisions, evidence, native_paths, and protected_skips. Always inspect compatibility, missing widgets/media, protected enhancement zones, structured_write_ready, and validation errors before merging the returned elementor_data into a page.

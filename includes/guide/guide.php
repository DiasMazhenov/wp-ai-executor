...
function wpae_agent_guide(): array {
    return [
        ...
        'chat_quality_contract' => [
            ...
            'visual_grammar' => 'Every generated block receives one compact outlined rounded native badge. Every repeatable card with a heading uses a native icon-box with an icon positioned to the left of that heading. This is a plugin default and must not be required in the user prompt.',
        ...
        'frontend_design' => [
            ...
            'design_quality_bar' => [
                ...
                'Generated block grammar is mandatory: one compact outlined rounded badge per block and a native icon beside every repeatable card heading.',
            ],
...
Current runtime notes: rollback restores only plugin-managed Elementor/WPAE meta plus the post record and does not remove unrelated third-party post meta; /elementor/page attempts to delete a newly created page if Elementor metadata saving fails and reports cleanup.created_post_deleted; /media/upload rejects files whose binary signature does not match mime_type; /exports/create stores short-lived JSON in wp_options and returns an authenticated /exports/{id} endpoint, never a public uploads file; use GET /exports for metadata and POST /exports/prune to remove expired export records.
   ...
   Generated component grammar note: every generated block gets one compact outlined rounded native badge, and every repeatable card heading gets a native icon-box with an icon positioned beside it. The plugin enforces this after provider decoding, so users do not need to write technical layout instructions.

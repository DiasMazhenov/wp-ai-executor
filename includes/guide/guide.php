...
function wpae_agent_guide(): array {
    return [
        ...
        'chat_quality_contract' => [
            ...
            'visual_grammar' => 'Every generated block receives one compact outlined rounded native badge container with a native heading label inside. Generic repeatable cards may use a native icon-box with a contrasting icon to the left of a short heading. Testimonial cards are an explicit exception: the author/company uses a native heading and the quote uses a native text-editor; an icon is decorative only and must never turn the author or quote into an icon-box. The bento grid container stays transparent; card backgrounds belong to the cards. This is a plugin default and must not be required in the user prompt.',
        ...
        'runtime_elementor_validation' => [
            ...
            'elementor_data_rules' => [
                ...
                'Generated blocks use one compact outlined rounded badge. Generic repeatable card headings may use native icon-box, but testimonial authors use native heading plus native text-editor quote and do not use icon-box.',
...
Current runtime notes: rollback restores only plugin-managed Elementor/WPAE meta plus the post record and does not remove unrelated third-party post meta; /elementor/page attempts to delete a newly created page if Elementor metadata saving fails and reports cleanup.created_post_deleted; /media/upload rejects files whose binary signature does not match mime_type; /exports/create stores short-lived JSON in wp_options and returns an authenticated /exports/{id} endpoint, never a public uploads file; use GET /exports for metadata and POST /exports/prune to remove expired export records.
   AI Vision note: when the ai_vision capability is enabled, render desktop and mobile screenshots in the agent environment, call /vision/analyze or submit a structured result to /vision/report, then call /vision/page-review. The floating Elementor chat reviews its refreshed preview through /llm/vision-review after each successful action in advisory mode and never rolls back a successful editor write from subjective screenshot findings. Vision returns vision_score, findings, confidence, and must_fix; it is additional evidence and never replaces deterministic audits, native Elementor checks, or public browser verification. Use transaction_vision_review=true with vision_report_id when atomic rollback on critical findings is explicitly required.
   ...
   Generated component grammar note: every generated block gets one compact outlined rounded native badge container with a heading label inside. Generic repeatable card headings may use a native icon-box with a contrasting icon, but testimonial cards are the semantic exception: author/company is a native heading, the quote is a native text-editor, and any quote/icon marker is decorative only. The card-grid container remains transparent. The plugin enforces this after provider decoding, so users do not need to write technical layout instructions.

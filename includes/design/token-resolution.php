...
function wpae_design_token_validate_contrast( array $tokens = [] ): array {
	$report = [ 'ok' => true, 'pairs' => [], 'errors' => [] ];
	$pairs = [
		[ 'foreground' => 'color.text', 'background' => 'color.page_bg', 'minimum' => 4.5 ],
		[ 'foreground' => 'color.muted', 'background' => 'color.page_bg', 'minimum' => 4.0 ],
		[ 'foreground' => 'color.surface', 'background' => 'color.primary', 'minimum' => 3.0 ],
	];
	foreach ( $pairs as $pair ) {
		...
		$entry = [ 'foreground' => $pair['foreground'], 'background' => $pair['background'], 'ratio' => $ratio, 'minimum' => $pair['minimum'] ];
		$report['pairs'][] = $entry;
		if ( $ratio === null || $ratio + 0.0001 < $pair['minimum'] ) {

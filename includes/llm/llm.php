...
function wpae_llm_chat( WP_REST_Request $request ) {
    ...
    if ( $action_request ) {
        ...
        if ( $decoded_action !== 'insert_elements' || count( $decoded_elements ) > 12 || $decoded_widget_count < 1 ) {
            $repair_messages = [
                [ 'role' => 'system', 'content' => 'Исправь Elementor action JSON. Верни только JSON без markdown и текста. Нужен ровно один верхнеуровневый elType=container с 3–5 заполненными native widget descendants: heading, text-editor и button. Каждый widget обязан иметь точный camelCase widgetType, elements: [], а также непустые native settings: heading обязан иметь непустой settings.title и header_size; text-editor обязан иметь непустой settings.editor; button обязан иметь непустой settings.text и settings.link.url. Не возвращай пустые title/editor/text, пустые контейнеры, плоские виджеты, дополнительные верхнеуровневые элементы, REST-маршруты или пояснения. Схема: {"action":"insert_elements","post_id":' . (string) $post_id . ',"position":"end","elements":[container]}. Используй именно post_id ' . (string) $post_id . '.' ],

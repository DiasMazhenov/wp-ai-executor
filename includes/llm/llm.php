...
function wpae_llm_chat( WP_REST_Request $request ) {
    ...
    if ( $action_request ) {
        ...
        if ( $decoded_action !== 'insert_elements' || count( $decoded_elements ) > 12 || $decoded_widget_count < 1 ) {
            $repair_error = '';
            $repair_messages = [
                [ 'role' => 'system', 'content' => 'Исправь Elementor action JSON. Верни только JSON без markdown и текста. Нужен ровно один верхнеуровневый elType=container с 3–5 заполненными native widget descendants. Используй именно post_id ' . (string) $post_id . '. Сгенерируй осмысленный русский контент под запрос пользователя «' . sanitize_text_field( $message ) . '», а не служебные заглушки. Обязательно верни минимум эти три объекта внутри container.elements: {"elType":"widget","widgetType":"heading","settings":{"title":"Преимущества, которые работают на задачу","header_size":"h2"},"elements":[]}, {"elType":"widget","widgetType":"text-editor","settings":{"editor":"Понятная структура и конкретная польза для клиента."},"elements":[]}, {"elType":"widget","widgetType":"button","settings":{"text":"Обсудить проект","link":{"url":"#contact"}},"elements":[]}. Не используй тексты «Заголовок блока», «Короткое описание результата для клиента», «Текст заголовка» или другие placeholder-фразы. У heading не может быть пустым settings.title, у text-editor settings.editor, у button settings.text или settings.link.url. Не возвращай пустые контейнеры, плоские виджеты, дополнительные верхнеуровневые элементы, REST-маршруты или пояснения. Схема: {"action":"insert_elements","post_id":' . (string) $post_id . ',"position":"end","elements":[container]}.' ],

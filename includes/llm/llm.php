...
function wpae_llm_extract_requested_content( string $message ): array {
	...
	if ( ! empty( $structured_pairs ) && wpae_llm_is_content_composition_request( $message ) && ! $process_request ) {
		foreach ( wpae_llm_content_units( $message ) as $unit ) {
			...
			if ( preg_match( '/^\s*(?:создай|создать|сделай|добавь|добавить|сформируй|собери|используй|примени|адаптируй|сохран\w*|не\s+выдум\w*|не\s+добавляй|в\s+кажд(?:ой|ом)\s+карточк|на\s+телефон\w*|на\s+мобильн\w*|дизайн\s+только\s+для\s+этого\s+нового\s+корня|существующ\w*\s+элемент\w*\s+страниц\w*\s+не\s+меняй)\b/iu', $unit ) ) {
				continue;
			}
			$normalized_unit = wpae_llm_normalize_content_text( $unit );
...
function wpae_llm_normalize_cta_url( $value ): string {
	return function_exists( 'wpae_brief_ir_normalize_url' ) ? wpae_brief_ir_normalize_url( $value ) : '';
}
...
function wpae_llm_execute_action( array $action, int $post_id, string $archetype = '', int $variation_seed = -1, string $message = '', bool $preserve_provider_design = false, array $operation_context = [] ): array {
	...
	if ( ! empty( $operation_context['replacement_guard'] ) ) {
		...
		$verified = is_array( $parent ) && function_exists( 'wpae_design_operation_replacement_target' )
			? wpae_design_operation_replacement_target( $parent, $post_id, (string) ( $guard['operation_identity'] ?? '' ), absint( $guard['revision'] ?? 0 ), (array) ( $guard['root_ids'] ?? [] ), $existing )
			: [ 'ok' => false, 'reason' => 'replacement_guard_unavailable' ];
		if ( empty( $verified['ok'] ) || $replace_root_ids !== (array) ( $verified['root_ids'] ?? [] ) ) {
			$steps[] = [ 'id' => 'replacement_ownership', 'status' => 'failed', 'message' => 'Сохранённый target replacement изменился; запись остановлена.', 'details' => [ 'reason' => $verified['reason'] ?? 'root_scope_mismatch' ] ];
			return [ 'ok' => false, 'operation_id' => $operation_id, 'error' => 'Replacement конфликтует с текущим сохранённым состоянием; чужие изменения не затронуты.', 'steps' => $steps ];
		}
	}
    $replace_index = null;
    $replace_element_id = '';
...
function wpae_llm_chat_request( WP_REST_Request $request ) {
	...
	if ( $operation_identity === '' ) {
		$operation_identity = 'client-' . substr( hash( 'sha256', $selected_post_id . '|' . microtime( true ) . '|' . wp_json_encode( $message ) ), 0, 24 );
	}
    $operation_identity = substr( $operation_identity, 0, 120 );
	...
	if ( $replacement_requested ) {
		...
		$parent_revision = absint( $replacement_context['revision'] ?? 0 );
		$parent_roots = array_values( array_filter( array_map( 'sanitize_key', array_slice( (array) ( $replacement_context['root_ids'] ?? [] ), 0, 12 ) ) ) );
		$replacement_parent = function_exists( 'wpae_design_operation_find_by_id' ) ? wpae_design_operation_find_by_id( $parent_id ) : null;
		$replacement_readback = $selected_post_id > 0 && function_exists( 'wpae_get_elementor_data_for_post' ) ? wpae_get_elementor_data_for_post( $selected_post_id ) : null;
		if ( ! is_array( $replacement_parent ) || ! is_array( $replacement_readback ) || ! function_exists( 'wpae_design_operation_replacement_target' ) ) {
	...
	if ( $replacement_requested && ( $design_pipeline_mode !== 'active' || ( $design_generation_route['action_path'] ?? '' ) !== 'pipeline' ) ) {
		return new WP_Error( 'wpae_vision_replacement_route_unavailable', 'Безопасный deterministic replacement недоступен в текущем режиме; root оставлен без изменений.', [ 'status' => 409, 'details' => [ 'mode' => $design_pipeline_mode, 'action_path' => $design_generation_route['action_path'] ?? 'unknown' ] ] );
	}
    $design_pipeline_trace = [
        'schema' => 'wpae-design-pipeline-trace-v1',
		'mode' => $design_pipeline_mode,
    ...
    if (
        ...
        if ( ! empty( $ir_compiled['ok'] ) && ! empty( $ir_compiled['elementor_data'] ) ) {
			$pipeline_operation_type = $replacement_requested ? 'design_repair' : 'design';
			$operation_ledger = function_exists( 'wpae_design_operation_create' ) ? wpae_design_operation_create( [
				'operation_id' => 'wpae-' . substr( hash( 'sha256', $operation_identity . '|' . (string) ( $design_pipeline_trace['brief']['hash'] ?? '' ) ), 0, 16 ),
				'idempotency_key' => function_exists( 'wpae_design_operation_idempotency_key' ) ? wpae_design_operation_idempotency_key( $selected_post_id, (string) ( $design_pipeline_trace['brief']['hash'] ?? '' ), 'page', $pipeline_operation_type, $operation_identity ) : '',
				'operation_identity' => $operation_identity,
				'operation_type' => $pipeline_operation_type,
			...
			$active_action = [ 'action' => 'insert_elements', 'post_id' => $selected_post_id, 'position' => $replacement_requested ? 'replace' : 'end', 'elements' => $ir_compiled['elementor_data'] ];
			$design_pipeline_trace['preflight'] = [
				'selected_scope' => $replacement_requested ? 'operation_owned_root' : ( ! empty( $selected_element_ids ) ? 'selected_elements' : 'page' ),
				'roots_to_add' => $replacement_requested ? 0 : count( (array) $ir_compiled['elementor_data'] ),
    ...
    if ( is_array( $context ) ) {
        ...
        $context = [
			...
			'operation_identity' => substr( sanitize_text_field( (string) ( $context['operation_identity'] ?? $context['request_id'] ?? '' ) ), 0, 120 ),
        ];
    ...
    if ( $action_request ) {
		...
		$routing_telemetry = [
			'source' => $design_engine_active ? 'local_edde' : 'provider_response',
			'action_path' => $resolved_action_path,
			'provider_calls' => $design_engine_active ? 0 : count( $provider_attempts ),
			'input_tokens' => $usage['input_tokens'] ?? null,
			'output_tokens' => $usage['output_tokens'] ?? null,
			'latency_ms' => null,
			'retry_count' => max( 0, count( $provider_attempts ) - 1 ),
			'fallback_used' => $used_fallback_model !== '' || $provider_transport_fallback,
			'success' => true,
		];
		$design_pipeline_trace['routing'] = function_exists( 'wpae_llm_route_diagnostics' ) ? wpae_llm_route_diagnostics( $routing_policy, $routing_telemetry ) : $routing_telemetry;
		$generation_diagnostics = [
            'schema' => 'wpae-llm-generation-diagnostics-v1',
            'archetype' => $action_archetype,
			'action_path' => $resolved_action_path,
			'routing' => function_exists( 'wpae_llm_route_diagnostics' ) ? wpae_llm_route_diagnostics( $routing_policy, $routing_telemetry ) : $routing_telemetry,
			'design_pipeline' => $design_pipeline_trace,
			'design_engine' => (array) ( $design_engine_result['trace'] ?? [] ),
			'provider_response' => $response_diagnostics,
...
function wpae_design_operation_reconcile_endpoint( WP_REST_Request $request ) {
	...
	if ( strlen( (string) wp_json_encode( $payload ) ) > 32768 ) {
		return new WP_Error( 'wpae_operation_payload_too_large', 'Доказательство операции превышает допустимый размер.', [ 'status' => 413 ] );
	}
	$operation_id = sanitize_key( (string) ( $payload['operation_id'] ?? '' ) );
	$post_id = absint( $payload['post_id'] ?? 0 );
	if ( $operation_id === '' || $post_id <= 0 || ! current_user_can( 'edit_post', $post_id ) ) {
		return new WP_Error( 'wpae_operation_reconcile_forbidden', 'Операция или право редактирования страницы не подтверждены.', [ 'status' => 403 ] );
	}
	$operation = function_exists( 'wpae_design_operation_find_by_id' ) ? wpae_design_operation_find_by_id( $operation_id ) : null;
	if ( ! is_array( $operation ) ) {
		return new WP_Error( 'wpae_operation_not_found', 'Операция не найдена.', [ 'status' => 404 ] );
	}
	if ( absint( $operation['post_id'] ?? 0 ) !== $post_id ) {
		return new WP_Error( 'wpae_operation_scope_mismatch', 'Операция принадлежит другой странице.', [ 'status' => 409 ] );
	}
	$identity = sanitize_text_field( (string) ( $payload['operation_identity'] ?? '' ) );
	$stored_identity = sanitize_text_field( (string) ( $operation['operation_identity'] ?? '' ) );
	if ( $stored_identity !== '' && ( $identity === '' || ! hash_equals( $stored_identity, $identity ) ) ) {
		return new WP_Error( 'wpae_operation_identity_mismatch', 'Устаревшее подтверждение относится к другой операции.', [ 'status' => 409 ] );
	}
	$requested_state = sanitize_key( (string) ( $payload['state'] ?? 'written' ) );
	if ( ! in_array( $requested_state, [ 'written', 'rendered', 'reviewed', 'completed' ], true ) ) {
		$requested_state = 'written';
	}
	$elementor_data = function_exists( 'wpae_get_elementor_data_for_post' ) ? wpae_get_elementor_data_for_post( $post_id ) : new WP_Error( 'wpae_operation_readback_unavailable', 'Read-back Elementor недоступен.' );
	if ( is_wp_error( $elementor_data ) || ! is_array( $elementor_data ) ) {
	...
	$current_revision = absint( $operation['revision'] ?? 1 );
	if ( $expected_revision > 0 && $expected_revision < $current_revision ) {
		$same_evidence = $incoming_evidence_hash !== '' && hash_equals( (string) ( $operation['evidence_hash'] ?? '' ), $incoming_evidence_hash );
		$same_or_older_state = wpae_design_operation_state_rank( (string) ( $operation['current_state'] ?? 'planned' ) ) >= wpae_design_operation_state_rank( $requested_state );
		if ( $same_evidence && $same_or_older_state ) {
			return new WP_REST_Response( [ 'ok' => true, 'idempotent' => true, 'operation' => $operation, 'saved_hash' => $current_saved_hash, 'state' => $operation['current_state'] ?? 'written' ], 200 );
		}
	...
	if ( in_array( $requested_state, [ 'rendered', 'reviewed', 'completed' ], true ) ) {
		if ( ! in_array( $evidence_source, [ 'preview', 'public_preview' ], true ) ) {
			return new WP_Error( 'wpae_operation_render_evidence_required', 'Для rendered/reviewed/completed требуется evidence из актуального preview.', [ 'status' => 409 ] );
		}

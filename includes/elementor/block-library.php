...
function wpae_block_library_retrieval_aliases( string $archetype ): array {
    $aliases = [
        ...
        'about' => [ 'about', 'company', 'компан', 'студи', 'о нас', 'о компании' ],
...
function wpae_block_library_retrieve_for_prompt( string $message, string $archetype = '' ): array {
    ...
    $result['selected'] = [
        ...
        'bundled_fixture_id' => (string) ( $selected['summary']['bundled_fixture_id'] ?? '' ),
...
function wpae_block_library_summary( array $record ): array {
    ...
    return [
        ...
        'bundled_fixture_id' => (string) ( $record['bundled_fixture_id'] ?? '' ),
        ...
        'compatibility' => [
            ...
            'unavailable_widget_types' => array_values( (array) ( $record['compatibility']['unavailable_widget_types'] ?? [] ) ),
...
function wpae_enqueue_elementor_block_library_editor(): void {
    ...
    $config = wp_json_encode( [
        ...
        'strings' => [
            ...
            'shortWidgets' => 'видж.',
            'importTemplates' => 'Импортировать JSON в библиотеку',
            'importing' => 'Импорт',
            'fileTooLarge' => 'Файл больше лимита 4 МБ.',
            'jsonOnly' => 'Поддерживаются только JSON-файлы.',
            'invalidJson' => 'Файл не содержит корректный объект JSON.',
            'importDescription' => 'Локальный импорт из набора Elementor. Оставлен черновиком до проверки совместимости и прав использования.',
            'unverifiedLicense' => 'Не проверена: импортирован из локального архива пользователя; проверьте условия до одобрения или распространения.',
            'localArchiveAttribution' => 'Источник и лицензия не подтверждены по данным файла.',
            'importComplete' => 'Импортировано шаблонов',
            'importFailed' => 'Ошибки',
            'unavailableWidgets' => 'Недоступные виджеты',
        ],
    ] );
    if ( is_string( $config ) ) {

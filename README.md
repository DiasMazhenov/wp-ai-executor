...
`context` редактора. Обычные вопросы остаются консультационными, а явные
команды вроде «создай hero» могут вставить только новые элементы при включенных
`llm_chat` и `elementor_writes`; данные проходят существующий update, preflight,
protected-zone, visual-regression и rollback pipeline. Удаление и полная замена
страницы не поддерживаются. Непригодный action JSON проходит ограниченный repair;
резервный шаблон допускается только после ошибки генерации и проверки содержания. В редакторе Elementor при включенном разрешении появляется
...
Генератор сохраняет принятую композицию модели: native Flexbox, крупную
типографику, заданные тексты, ссылки и палитру. Библиотечный шаблон больше
не заменяет корректный ответ. Недостающие mobile/tablet-настройки дополняются,
а явно заданные значения сохраняются. Пустые деревья и композиции больше
80 элементов отправляются на repair. Цвета проекта служат исходной палитрой;
пользовательские цвета в native controls допустимы.

Все обращения к провайдеру в одной генерации делят бюджет 120 секунд.
При истечении времени браузера результат записи считается неизвестным:
повторная генерация автоматически не запускается. Отмена проверяет,
что после операции страницу не изменили, и обновляет весь редактор.

Проверки разработчика: `node --test tests/*.test.js` (нужны Node.js и PHP 8+).
PHP-сценарии используют реальные генератор, нормализатор, design contract,
